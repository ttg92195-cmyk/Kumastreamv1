...
function AdminDashboardContent() {
  ...
  // Change tab and reset to page 1
  ...
  const renderPagination = (totalPages: number, total: number) => {
    ...
    return (
      <div className="flex items-center justify-center gap-0.5 py-4 mt-4">
        <button
          onClick={() => goToPage(currentPage - 1)}
          disabled={currentPage === 1}
          className="w-10 h-10 rounded-lg bg-gray-700 text-white text-sm font-medium disabled:opacity-40 hover:bg-gray-600 transition-colors flex items-center justify-center"
        >
        ...
        <div className="flex items-center gap-0.5">
          {pages.map((page, idx) => (
            ...
            ) : <span key={idx} className="text-gray-500 px-2 text-sm">...</span>
          ))}
        </div>
        <button
          onClick={() => goToPage(currentPage + 1)}
      ...
      <div className="p-4">
        ...
        {!loading && (
          <div className="text-gray-400 text-xs mb-2">
            {activeTab === 'movies' && `Showing ${paginatedMovies.length} of ${filteredMovies.length} movies • Page ${currentPage} of ${movieTotalPages || 1}`}
            {activeTab === 'series' && `Showing ${paginatedSeries.length} of ${filteredSeries.length} series • Page ${currentPage} of ${seriesTotalPages || 1}`}
          </div>
        )}
