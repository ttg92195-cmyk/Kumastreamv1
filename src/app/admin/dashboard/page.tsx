...
import { Plus, Film, Tv, Grid3X3, Users, LogOut, Edit, Trash2, Loader2, RefreshCw, FileText, MoreVertical, Eye, Search, ChevronLeft, ChevronRight, Check, X } from 'lucide-react';
...
const staticPosts: Post[] = [
  { id: '1', title: 'Welcome to KUMASTREAM', excerpt: 'Get started with our streaming platform...', date: '2024-01-15', status: 'published' },
  { id: '2', title: 'New Features Released', excerpt: 'Check out the latest updates and features...', date: '2024-01-10', status: 'published' },
  { id: '3', title: 'Upcoming Movies in 2024', excerpt: 'A preview of the most anticipated movies...', date: '2024-01-05', status: 'draft' },
];
...
export default function AdminDashboardPage() {
  ...
  const [activeTab, setActiveTab] = useState<'movies' | 'series' | 'posts'>('movies');
  ...
  const getPaginatedData = (data: (Movie | Series | Post)[]) => {
  ...
  const handlePostAction = (postId: string, action: 'edit' | 'delete' | 'view') => {
    setOpenDropdown(null);
    if (action === 'edit') {
    ...
    <div className="min-h-screen bg-[#0f0f0f]">
      ...
      <div className="p-4">
        ...
        <div className="grid grid-cols-5 gap-3 mb-6">
          ...
          <div className="p-4 bg-gray-800 rounded-lg text-center">
            <FileText className="w-6 h-6 mx-auto mb-2" style={{ color: themeColor }} />
            <p className="text-white text-xl font-bold">{stats.posts}</p>
            <p className="text-gray-500 text-xs">Posts <span className="text-xs px-1.5 py-0.5 bg-gray-700 rounded ml-1">Static</span></p>
          </div>
          <div className="p-4 bg-gray-800 rounded-lg text-center">
            <Grid3X3 className="w-6 h-6 mx-auto mb-2" style={{ color: themeColor }} />
        ...
        <div className="flex items-center gap-2 mb-4 flex-wrap">
          ...
          <button
            ...
            style={activeTab === 'posts' ? { backgroundColor: themeColor } : {}}
          >
            Posts ({staticPosts.length})
          </button>
        ...
        {!loading && (
          <div className="text-gray-400 text-xs mb-2">
            ...
            {activeTab === 'posts' && `Showing ${paginatedPosts.length} of ${staticPosts.length} posts`}
          </div>
        )}
          ...
          <div className="space-y-2">
            ...
            {activeTab === 'posts' && (
              <div className="text-center py-10 bg-gray-800/50 rounded-lg">
                ...
                <p className="text-gray-400 text-sm">Static Posts</p>
                <p className="text-gray-500 text-xs mt-1">Posts are managed statically</p>
              </div>
            )}
                  ...
                  <div className="flex items-center gap-2" ref={dropdownRef}>
                    ...
                    <div className="relative">
                      ...
                      {openDropdown === post.id && (
                        <div className="absolute right-0 top-full mt-1 w-36 bg-gray-900 border border-gray-700 rounded-lg shadow-lg z-10">
                          <button
                            onClick={() => handlePostAction(post.id, 'edit')}
            ...
            {activeTab === 'posts' && renderPagination(postsTotalPages, staticPosts.length)}
          </div>
        )}
      </div>
    </div>
