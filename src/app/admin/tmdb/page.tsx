...
import { ArrowLeft, Search, Check, Loader2, Download, Film, Tv, ChevronLeft, ChevronRight, CheckCircle, Eye, EyeOff, ArrowUpDown, ArrowUp, ArrowDown } from 'lucide-react';
...
export default function TMDBGeneratorPage() {
  ...
  // Filter toggle - Hide imported items
  const [hideImported, setHideImported] = useState(true);
  
  // Get filtered results based on hideImported toggle
  ...
  }, [type, year, genre, count, searchQuery, sortBy, checkImportedItems]);
  ...
  const selectAllResults = () => {
    const availableIds = filteredResults
      .filter(r => !importedIds.has(r.id))
      .map(r => r.id);
  ...
  return (
    <div className="min-h-screen bg-[#0f0f0f]">
      ...
      <div className="p-4 space-y-4">
            ...
            <div className="col-span-2">
              ...
              <div className="flex gap-2 items-center">
                <input
                  ...
                  min="1"
                  className="flex-1 bg-gray-700 text-white rounded-lg px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-red-500"
                />
                {/* Quick Count Buttons */}
        ...
        {/* Stats & Filter Toggle */}
        {results.length > 0 && (
          <div className="flex items-center justify-between bg-gray-800/30 rounded-lg px-4 py-3 flex-wrap gap-2">
            <div className="flex items-center gap-4 text-sm">
              <span className="text-white">Total: <span className="font-bold">{totalResults}</span></span>
              <span className="text-green-500">✨ New: <span className="font-bold">{availableCount}</span></span>
            ...
            >
              {hideImported ? (
                <>
                  ...
                  Hiding Imported ({availableCount} shown)
                </>
              ) : (
                <>
                  <Eye className="w-4 h-4" />
                  Showing All ({totalResults} shown)
                </>
              )}
            </button>
          </div>
              ...
              {paginatedResults.map((item) => {
                ...
                return (
                  <div
                    ...
                    className={cn(
                      'relative rounded-md overflow-hidden transition-all cursor-pointer',
                  ...
                  >
                    <div className="aspect-[2/3] relative">
                      ...
                      {/* "NEW" Badge */}
                      {!isImported && (
                        <div className="absolute top-1.5 left-1.5 bg-green-500 text-white text-[10px] font-bold px-1.5 py-0.5 rounded">
                          NEW
                        </div>
                      )}
        ...
        {!loading && results.length > 0 && filteredResults.length === 0 && (
          <div className="text-center py-20">
            ...
            <p className="text-gray-400 text-sm mb-4">
              All {totalResults} items from this search are already in your database.
            </p>
            <button
