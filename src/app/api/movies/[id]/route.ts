...
export async function GET(
  ...
  try {
    ...
    try {
      ...
      const allMovies = dbMovies.map(normalizeMovie);
      ...
      const calculateGenreSimilarity = (movieGenres: string[], otherGenres: string): number => {
        const otherGenreList = (otherGenres || '').split(',').filter(Boolean).map((g: string) => g.trim().toLowerCase());
        return movieGenres.filter((g: string) => otherGenreList.some((og: string) => og === g.trim().toLowerCase())).length;
      };

      // Get genres of current movie
      const genres = (movie.genres || '').split(',').filter(Boolean).map((g: string) => g.trim());

      // Step 1: Get movies with genre similarity, sorted by similarity score then rating
      const moviesWithSimilarity = allMovies
        .filter((m: any) => m.id !== id)
        .map((m: any) => ({
          ...m,
          similarityScore: calculateGenreSimilarity(genres, m.genres)
        }))
        .sort((a: any, b: any) => {
      ...
      // Step 2: Get similar movies (those with at least one matching genre)
      let similarMovies = moviesWithSimilarity.filter((m: any) => m.similarityScore > 0).slice(0, 6);

      // Step 3: If not enough similar movies, fill with diversity (high-rated random movies)
      if (similarMovies.length < 6) {
        const remainingSlots = 6 - similarMovies.length;
        const existingIds = new Set(similarMovies.map((m: any) => m.id));
        existingIds.add(id); // Exclude current movie

        // Get diverse movies (high rating, not already included)
        const diverseMovies = allMovies
          .filter((m: any) => !existingIds.has(m.id))
          .sort((a: any, b: any) => (b.rating || 0) - (a.rating || 0))
          .slice(0, remainingSlots);

        similarMovies.push(...diverseMovies);
      }
