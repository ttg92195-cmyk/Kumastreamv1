...
export async function GET(
  ...
  try {
    ...
    try {
      ...
      const calculateGenreSimilarity = (seriesGenres: string[], otherGenres: string): number => {
        const otherGenreList = (otherGenres || '').split(',').filter(Boolean).map((g: string) => g.trim().toLowerCase());
        return seriesGenres.filter((g: string) => otherGenreList.some((og: string) => og === g.trim().toLowerCase())).length;
      };

      // Get genres of current series
      const genres = (series.genres || '').split(',').filter(Boolean).map((g: string) => g.trim());

      // Step 1: Get series with genre similarity, sorted by similarity score then rating
      const seriesWithSimilarity = allSeries
        .filter((s: any) => s.id !== id)
        .map((s: any) => ({
          ...s,
          similarityScore: calculateGenreSimilarity(genres, s.genres)
        }))
        .sort((a: any, b: any) => {
      ...
      // Step 2: Get similar series (those with at least one matching genre)
      let similarSeries = seriesWithSimilarity.filter((s: any) => s.similarityScore > 0).slice(0, 6);

      // Step 3: If not enough similar series, fill with diversity (high-rated random series)
      if (similarSeries.length < 6) {
        const remainingSlots = 6 - similarSeries.length;
        const existingIds = new Set(similarSeries.map((s: any) => s.id));
        existingIds.add(id); // Exclude current series

        // Get diverse series (high rating, not already included)
        const diverseSeries = allSeries
          .filter((s: any) => !existingIds.has(s.id))
          .sort((a: any, b: any) => (b.rating || 0) - (a.rating || 0))
          .slice(0, remainingSlots);

        similarSeries.push(...diverseSeries);
      }
