...
async function executeSQL(prisma: PrismaClient, sql: string, description: string) {
  try {
...
export async function GET() {
  ...
  try {
    ...
    for (const table of tables) {
      const result = await executeSQL(prisma, table.sql, `Create ${table.name} table`);
      if (result.success) {
    ...
    for (const indexSql of indexes) {
      await executeSQL(prisma, indexSql, 'Create index');
    }
    ...
    for (const fkSql of foreignKeys) {
      await executeSQL(prisma, fkSql, 'Create foreign key');
    }
    ...
    try {
      ...
      const existingUsers = await prisma.$queryRaw`SELECT * FROM "User" WHERE username = 'Admin8676'`;
