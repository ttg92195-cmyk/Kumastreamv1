...
export async function GET(request: Request) {
  try {
    ...
    const limit = count; // No limit - allow any number

    console.log('TMDB Search params:', { type, year, genre, count: limit, query });
    ...
    if (query) {
      ...
      let baseUrl = `${TMDB_BASE_URL}/discover/${type}?api_key=${TMDB_API_KEY}&sort_by=popularity.desc`;
