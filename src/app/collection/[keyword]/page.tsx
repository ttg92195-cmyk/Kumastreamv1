...
const UNIVERSE_KEYWORDS: Record<string, string[]> = {
  ...
  'x-men': [
    'X-Men', 'Wolverine', 'Deadpool', 'Logan'
  ],
  'spider-man': [
    'Spider-Man', 'Spiderman', 'Venom', 'Morbius', 'Madame Web', 'Kraven'
  ],
  ...
  // Additional Universe Collections
  'james bond': [
    'James Bond', '007', 'Bond', 'Skyfall', 'Spectre', 'No Time to Die',
    'Casino Royale', 'Quantum of Solace', 'Die Another Day', 'GoldenEye',
    'Tomorrow Never Dies', 'Thunderball', 'Goldfinger', 'Dr. No',
    'From Russia with Love', 'You Only Live Twice', 'On Her Majesty',
    'The Spy Who Loved Me', 'Moonraker', 'For Your Eyes Only', 'Octopussy',
    'A View to a Kill', 'The Living Daylights', 'Licence to Kill', 'Never Say Never Again'
  ],
  'fast & furious': [
    'Fast & Furious', 'Fast and Furious', 'Furious', 'Fast Five', 'Hobbs & Shaw',
    'Fast X', 'F9', 'The Fate of the Furious', 'Tokyo Drift', '2 Fast', 'Los Bandoleros'
  ],
  'mission impossible': [
    'Mission Impossible', 'Mission: Impossible'
  ],
        ...
        if (universeKeywords) {
          ...
          const fetchPromises = universeKeywords.map(async (searchKeyword) => {
            const [moviesRes, seriesRes] = await Promise.all([
              fetch(`/api/movies?search=${encodeURIComponent(searchKeyword)}&limit=50`),
              fetch(`/api/series?search=${encodeURIComponent(searchKeyword)}&limit=50`)
            ]);
          ...
          const moviesRes = await fetch(`/api/movies?search=${encodeURIComponent(keyword)}&limit=100`);
          if (moviesRes.ok) {
            const data = await moviesRes.json();
          ...
          const seriesRes = await fetch(`/api/series?search=${encodeURIComponent(keyword)}&limit=100`);
          if (seriesRes.ok) {
            const data = await seriesRes.json();
