...
// Predefined Collections (searches by title keyword)
const AUTO_COLLECTIONS = [
  ...
  // Moved from Manual Collections
  '007', 'Lord of the Rings', 'A24'
];

// Universe collections that search multiple related keywords
const UNIVERSE_COLLECTIONS = ['Marvel', 'DC', 'The Conjuring Universe', 'Jurassic', 'MonsterVerse', 'Despicable Me', '007', 'Lord of the Rings'];

// Additional Collections (from database collection field - for manually tagged content)
const DB_COLLECTIONS = [
  'CHRISTMAS MOVIES', 'Dragon Gate Posthouse',
...
function GenresContent() {
  ...
  // Check if it's a special value for showing tabs (not actual filtering)
  ...
  // activeTab: 'tags' when showing tags list, 'collections' when showing collections list
  ...
  const fetchResults = async () => {
    ...
    try {
      // Only fetch if we have real filter values (not special _show_ values)
      const actualTag = isShowTags ? null : tag;
      const actualCollection = isShowCollections ? null : collection;
      
      if (actualTag) {
        const moviesRes = await fetch(`/api/movies?tag=${encodeURIComponent(actualTag)}&limit=9999`);
        if (moviesRes.ok) {
          const data = await moviesRes.json();
          setMovies(data.movies || []);
        }
        
        const seriesRes = await fetch(`/api/series?tag=${encodeURIComponent(actualTag)}&limit=9999`);
        if (seriesRes.ok) {
          const data = await seriesRes.json();
          setSeries(data.series || []);
        }
      } else if (actualCollection) {
        const moviesRes = await fetch(`/api/movies?collection=${encodeURIComponent(actualCollection)}&limit=9999`);
        if (moviesRes.ok) {
          const data = await moviesRes.json();
          setMovies(data.movies || []);
        }
        
        const seriesRes = await fetch(`/api/series?collection=${encodeURIComponent(actualCollection)}&limit=9999`);
        if (seriesRes.ok) {
          const data = await seriesRes.json();
          setSeries(data.series || []);
        }
  ...
  return (
    <div className="p-4 space-y-6">
      {/* Active Filter - only show when real filter is active */}
      {hasRealFilter && (
      ...
      {/* Results Section - only show when real filter is active */}
      {hasRealFilter && (
      ...
      {/* Tabs Selection (only show when no real filter active) */}
      {!hasRealFilter && (
        <>
          {/* Tabs */}
                ...
                <div className="grid grid-cols-3 gap-2">
                  {genres.movies.map((genre) => (
                    <button
                      ...
                      className="px-3 py-3 bg-gray-800 border border-gray-700 text-gray-300 rounded-lg text-sm font-medium hover:bg-gray-700 hover:border-gray-600 transition-all"
                    >
                      {genre.name}
                    </button>
                  ))}
                ...
                <div className="flex flex-wrap gap-2">
                  {MOVIE_TAGS.map((t) => (
                    <button
                      ...
                      className="px-3 py-2 bg-red-500/20 border border-red-500/50 text-red-400 rounded-lg text-sm font-medium hover:bg-red-500/30 hover:border-red-400 transition-all"
                    >
                      {t}
                    </button>
                  ))}
          ...
          {activeTab === 'collections' && (
            <div className="space-y-6">
              {/* Auto Collections - Search by Title Keyword */}
              <section>
              ...
              {/* DB Collections - Filter by Collection Field */}
              <section>
                <div className="flex items-center gap-2 mb-2">
                  <FolderOpen className="w-4 h-4" style={{ color: themeColor }} />
...
// Header component that uses useSearchParams
