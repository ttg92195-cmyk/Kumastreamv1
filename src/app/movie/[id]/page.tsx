...
import { ArrowLeft, Heart, Star, Calendar, Clock, ChevronDown, Download, AlertCircle, Lock, Server, ExternalLink, ChevronUp, X } from 'lucide-react';
...
export default function MovieDetailPage() {
  ...
  const getServerGroups = (links: { server: string; quality: string; url: string; size?: string | null }[]) => {
    // Group links by actual server name from database
    ...
    return Object.entries(serverMap)
      .sort(([a], [b]) => {
        // Extract numbers from server names for proper sorting
  ...
  // Get default servers when no custom links exist
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#0f0f0f]">
        ...
        <div className="relative w-full h-[280px] bg-gray-700/50">
          <div className="absolute inset-0 bg-gradient-to-t from-[#0f0f0f] via-[#0f0f0f]/60 to-transparent" />
        </div>
        <div className="px-4 -mt-16 relative z-10">
          <div className="flex items-end gap-4 mb-4">
            <div className="w-24 h-36 bg-gray-700/50 rounded-lg flex-shrink-0" />
  ...
  const genreList = movie.genres ? movie.genres.split(',').filter(Boolean) : [];
  const posterUrl = movie.poster || PLACEHOLDER_POSTER;
  const backdropUrl = movie.backdrop || posterUrl;
  
  // Get server groups for download
  ...
  const renderTabContent = () => {
    switch (activeTab) {
      ...
      case 'download':
          ...
          return (
            <div className="space-y-4">
              ...
              <div className="text-center py-10 bg-gray-800/50 rounded-lg border border-gray-700">
                <Lock className="w-12 h-12 mx-auto mb-3" style={{ color: themeColor }} />
        ...
        // Show download button that opens modal
        return (
          <div className="space-y-4">
            <h3 className="text-white font-semibold">Download Options</h3>
            <button
              onClick={() => setShowDownloadModal(true)}
              className="w-full py-4 bg-gray-800/50 rounded-lg border border-gray-700 flex items-center justify-center gap-3 hover:bg-gray-800 transition-colors"
            >
              <Download className="w-5 h-5" style={{ color: themeColor }} />
              <span className="text-white font-medium">View Download Links</span>
              <span className="text-gray-400 text-sm">
                ({serverGroups.length > 0 ? serverGroups.length : defaultServers.length} servers)
              </span>
            </button>
          </div>
        );
        ...
        <div className="absolute top-4 left-4 right-4 flex items-center justify-between">
          ...
          <button
            ...
            className={cn(
              ...
              bookmarked ? 'text-white' : 'bg-black/50 text-white'
            )}
            style={bookmarked ? { backgroundColor: themeColor } : {}}
          >
        ...
        <div className="flex items-end gap-4 mb-4">
          ...
          <div className="relative w-24 h-36 flex-shrink-0 rounded-lg overflow-hidden shadow-xl">
            ...
            {!movie.quality && movie.quality4k && (
              <div className="absolute top-1 left-1 text-white text-[10px] font-bold px-1.5 py-0.5 rounded-full" style={{ backgroundColor: themeColor }}>
                4K
              </div>
            )}
          </div>
                        ...
                        {server.links.map((link, linkIndex) => (
                          <div key={linkIndex} className="p-3 bg-gray-700/50 rounded-lg">
                            ...
                            <a
                              ...
                              className="w-full py-2.5 text-white rounded-lg text-sm font-medium flex items-center justify-center gap-2 transition-colors hover:opacity-90"
                              style={{ backgroundColor: themeColor }}
                            >
