  ...
  return (
    <Link href={`/series/${series.id}`} className="group block">
      <div className="relative aspect-[2/3] rounded-lg overflow-hidden bg-neutral-800">
        ...
        {/* 4K Badge */}
        {series.quality4k && (
          <div className="absolute top-2 left-2 w-8 h-8 rounded-full flex items-center justify-center" style={{ backgroundColor: currentColor }}>
        ...
        <div className="absolute bottom-2 right-2 flex items-center gap-1 bg-black/70 px-2 py-1 rounded-md">
          <Star className="w-3 h-3" style={{ color: currentColor, fill: currentColor }} />
          <span className="text-white text-xs font-medium">{series.rating.toFixed(1)}</span>
        </div>
        
        {/* Seasons Badge */}
        {series.seasons && (
          <div className="absolute bottom-2 left-2 px-2 py-0.5 rounded text-xs text-white font-medium" style={{ backgroundColor: `${currentColor}cc` }}>
            S{series.seasons}
          </div>
        )}
        ...
        >
          {bookmarked ? (
            <BookmarkCheck className="w-4 h-4" style={{ color: currentColor }} />
          ) : (
